...
curl -s "$BASE_URL/syllabus/" -X POST -H "Content-Type: application/json" \
...
curl -s "$BASE_URL/syllabus/$SYLLABUS_ID/assessment/" -X POST -H "Content-Type: application/json" \
  -d '{"num_questions": 4}' \
...
curl -s "$BASE_URL/assessment/$ASSESSMENT_ID/grade/" -X POST -H "Content-Type: application/json" \
...
curl -s "$BASE_URL/assessment/$ASSESSMENT_ID/roadmap/" -X POST -H "Content-Type: application/json" \
  -d '{}' \
